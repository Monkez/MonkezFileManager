...
## Tùy Chỉnh Thanh Giao Diện

- Rê chuột vào vùng phân cách dưới Topbar để hiện nút mũi tên ở giữa, sau đó nhấn để thu gọn thanh menu trên cùng. Khi Topbar đang ẩn, rê chuột sát mép trên cửa sổ để hiện nút mở lại.
- Mở **Settings → Thành Phần Giao Diện** để hiện/ẩn **Status Bar**.
- Có thể tắt riêng dải gợi ý `F2`, `F5`, `F6`, `F7`, `DEL` mà vẫn giữ thông tin pane và đường dẫn trên Status Bar.
- Trạng thái Topbar, Status Bar và gợi ý phím tắt được ứng dụng ghi nhớ cho lần mở sau.
- Hai nút cuộn trái/phải trên thanh tab chỉ hiện khi danh sách tab dài hơn vùng hiển thị.
...
- Nhấn chuột phải vào file, mở nhóm **Windows Shell** để xem tác vụ hệ thống hoặc nhóm **Quản lý tệp** để chọn **Tạo shortcut tại đây**.
