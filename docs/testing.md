...
- `backend/tests/taskManager.test.js`: kiểm tra copy task, tiến độ hoàn thành và xử lý trùng tên không ghi đè.
...
Khi sửa logic file system, cần thêm test cho edge case tương ứng: path không tồn tại, tên không hợp lệ, trùng tên, copy/move thư mục, batch rename, undo/redo, cancel/pause/resume task và lỗi quyền truy cập.
